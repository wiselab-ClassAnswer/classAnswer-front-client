...
export const GridUtil = {
    ...
    onPagingCommon: function(pageData, gridObj, callback) {
     ...
     /**
     * 그리드 행 스크롤 
     * 그리드 rowId를 입력받아 해당 rowId를 가진 행을 맨 위에 위치하게 함.
     * @param gridObj 
     * @param rowId
     *  
     */
    srcollToRow(gridObj, rowId){
        let rowPosition = gridObj.find(`#${rowId}`).position();
        if (rowPosition) {
            gridObj.closest('.ui-jqgrid-bdiv').scrollTop(rowPosition.top);
        }
    },
     ...
     * 
     * Key로 인덱스 찾기
     * 두개의 그리드중 한 그리드의 Key 값을 활용해서 다른 그리드의 행 번호 찾기 
     */
    findIndexToKey(frstGridObj,frstGridRowId ,scndGridObj, key){
        
        let frstRowData = frstGridObj.jqGrid('getRowData',frstGridRowId);
        let scndRowData = scndGridObj.jqGrid('getGridParam','data');

        // 첫번째 그리드에서 선택한 로우의 key를 찾음 
        let slctFrstRowData = scndRowData.find(el => 
            Object.keys(frstRowData).every(key => el[key] === frstRowData[key])
        );

        let slctRowId = null;
        
        if(ValdUtil.isNotNull(slctFrstRowData)){
            // 첫번쨰 그리드와 동일한 Key를 갖는 두번째 그리드의 행 번호
            slctRowId = scndRowData.findIndex(el => 
                Object.keys(slctFrstRowData).every(key => el[key] === slctFrstRowData[key])
            ) + 1;
            // jqGrid는 0부터 시작하기 때문에 +1 
        }
